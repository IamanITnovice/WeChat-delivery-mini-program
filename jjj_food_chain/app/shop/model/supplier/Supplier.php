    ...
    public function edit($data)
    {
        ...
        try {
            ...
            if (isset($supplier['password']) && !empty($supplier['password'])) {
                if (!isset($supplier['confirm_password']) || $supplier['confirm_password'] !== $supplier['password']) {
                    $this->error = '确认密码和登录密码不一致';
                    return false;
                }
                if (!PasswordService::checkStrength($supplier['password'], $error)) {
                    $this->error = $error;
                    return false;
                }
                $user_data['password'] = PasswordService::hash($supplier['password']);
            }
