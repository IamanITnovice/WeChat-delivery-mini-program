...
class AuthService
{
    ...
    private function getAccessUrls()
    {
        if (empty($this->accessUrls)) {
            ...
            $cacheKey = 'user_access_urls_' . $shop_user_id;

            // 尝试从缓存获取
            $this->accessUrls = Cache::get($cacheKey);
            ...
            if (empty($this->accessUrls)) {
                ...
                // 缓存权限URL列表（1小时）
                Cache::set($cacheKey, $this->accessUrls, 3600);
            }
        }
    ...
    public static function getAccessNameByPath($path, $app_id)
    {
        ...
        if (!$arr) {
            ...
            Cache::set('path_access_' . $app_id, $arr, 3600);
        }
