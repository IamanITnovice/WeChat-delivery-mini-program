...
class Product extends BaseModel
{
    ...
    //计算商品价格
    public function getCartPrice($param)
    {
        $product = $this->where('product_id', '=', $param['product_id'])->find();
        $skuInfo = (new ProductSku())->where('product_id', '=', $param['product_id'])
            ->where('product_sku_id', '=', $param['product_sku_id'])
            ->find();
        if (!$product || !$skuInfo) {
            return false;
        }
        $product_price = $skuInfo['product_price'];
        $price = $product_price;
        ...
        if ($feed) {
            ...
            foreach ($feed as $item) {
                foreach ($product['product_feed'] as $value) {
                    if ($item == $value['feed_name']) {
                        $feed_price += $value['price'];
                        break;
                    }
                }
            }
        ...
        $data = [
            'product_price' => round($product_price + $feed_price, 2),
            'price' => round($price + $feed_price, 2),
            'bag_price' => $skuInfo['bag_price'],
        ];
        return $data;
    }
}
