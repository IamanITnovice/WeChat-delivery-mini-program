...
class Cart extends CartModel
{
    ...
    public function add($data, $user)
    {
        ...
        if ($cartInfo) {
            ...
            $cartPrice = (new ProductModel())->getCartPrice($data);
            if (!$cartPrice) {
                $this->error = '商品规格不存在';
                return false;
            }
            $data['price'] = $cartPrice['price'];
            $data['product_price'] = $cartPrice['product_price'];
            $data['bag_price'] = $cartPrice['bag_price'];
     ...
     * 加减商品
     */
    public function sub($param, $user = null)
    {
        // 验证用户归属，防止越权操作
        if ($user && $this['user_id'] != $user['user_id']) {
            $this->error = '无权操作';
            return false;
        }
        //判断是否营业
