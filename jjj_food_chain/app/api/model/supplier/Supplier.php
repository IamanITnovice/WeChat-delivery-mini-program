...
class Supplier extends SupplierModel
{
    ...
    {
        $param['latitude'] = isset($param['latitude']) && is_numeric($param['latitude']) ? $param['latitude'] : 0;
        $param['longitude'] = isset($param['longitude']) && is_numeric($param['longitude']) ? $param['longitude'] : 0;
    ...
    public function deliveryList($param, $user)
    {
        ...
        if ($param['type'] == 10) {
            ...
            $latitude = isset($param['latitude']) && is_numeric($param['latitude']) ? $param['latitude'] : 0;
            $longitude = isset($param['longitude']) && is_numeric($param['longitude']) ? $param['longitude'] : 0;
        }
        // 查询列表数据
        $list = $model->alias('s')
            ->where('is_recycle', '=', 0)
