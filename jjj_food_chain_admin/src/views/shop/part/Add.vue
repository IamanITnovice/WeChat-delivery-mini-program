  ...
  <el-dialog title="新增平台" v-model="dialogVisible" @close="dialogFormVisible" :close-on-click-modal="false"
    ...
    <el-form size="small" :model="form" ref="form" :rules="rules">
      ...
      <el-form-item label="平台账户密码" :label-width="formLabelWidth" prop="password">
        ...
        <p class="gray">注：平台后台用户密码，需为 8-20 位且至少包含字母、数字、符号中的 2 种</p>
      </el-form-item>
    ...
    <template #footer>
      <div class="dialog-footer">
        ...
        <el-button type="primary" @click="addClick()" :loading="loading">确定</el-button>
      </div>
    </template>
  </el-dialog>
</template>
    ...
    data() {
      ...
      let checkUsername = (rule, value, callback) => {
        if (!this.$filter.replaceSpace(value)) {
          return callback(new Error('平台账户名不能为空'));
        } else {
