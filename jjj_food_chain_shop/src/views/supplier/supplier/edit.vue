<template>
	...
	<div class="product-add">
		<el-form size="small" ref="form" :model="form" :rules="rules" label-width="200px">
			...
			<el-form-item label="门店名称" prop="supplier.name" :rules="[{required: true,message: '请输入门店名称'}]">
				<el-input class="max-w460" v-model="form.supplier.name" placeholder="请输入门店名称"></el-input>
			</el-form-item>
			<el-form-item label="账号" prop="supplier.user_name" :rules="[{required: true,message: '请输入账号'}]">
				<el-input
					:disabled="form.supplier.is_main==1"
					class="max-w460"
			...
			<el-form-item label="联系人" prop="supplier.link_name" :rules="[{required: true,message: '请输入联系人'}]">
				<el-input class="max-w460" v-model="form.supplier.link_name" placeholder="请输入联系人"></el-input>
			</el-form-item>
			<el-form-item label="联系电话" prop="supplier.link_phone" :rules="[{required: true,message: '请输入联系电话'}]">
			...
			<el-form-item label="门店 logo">
				...
				<span class="gray9 f12">建议尺寸 800px * 800px</span>
			</el-form-item>
			<el-form-item label="地址" prop="supplier.address" :rules="[{required: true,message: '请输入地址'}]">
				<el-input class="max-w460" v-model="form.supplier.address" placeholder="请输入地址"></el-input>
			</el-form-item>
			<el-form-item label="门店坐标" prop="supplier.coordinate" :rules="[{required: true,message: '请输入门店坐标'}]">
				...
				<div class="tips" style="color: red;">
					未申请腾讯地图 key，可在腾讯地图里搜索地址获取经纬度，示例经度 70.002749,116.323467
				</div>
			</el-form-item>
	...
	export default {
		...
		data() {
			...
			let checkPasswordConfirm = (rule, value, callback) => {
				if (!this.form.supplier.password) {
					callback();
				} else if (!value) {
					callback(new Error('请输入确认密码'));
				} else if (value !== this.form.supplier.password) {
